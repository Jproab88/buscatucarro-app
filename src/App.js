    ...
    return (
        <div className="w-full overflow-x-auto bg-white rounded-2xl shadow-xl border border-gray-200">
            <table className="min-w-full border-collapse">
                <colgroup>
                    <col className="w-48" />
                    {cars.map(car => <col key={car.id} className="w-64" />)}
                </colgroup>
                <thead>
                    <tr className="bg-gray-50">
                        <th className="sticky left-0 bg-gray-100 p-4 font-semibold text-gray-600 text-left text-sm tracking-wider">Vehículo</th>
                        {cars.map(car => (
                            <th key={car.id} className="p-4 font-semibold text-gray-600 border-l border-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                    {features.map((feature, fIndex) => (
                        <tr key={feature.key} className={fIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                            <td className="sticky left-0 p-4 font-medium text-gray-800 align-top bg-white border-r border-gray-200"><div className="flex items-center"><feature.icon className={`w-5 h-5 mr-3 ${feature.color} flex-shrink-0`} />{feature.label}</div></td>
                            {cars.map(car => (<td key={car.id} className="p-4 text-gray-600 align-top border-l border-gray-200 text-sm">{Array.isArray(car[feature.key]) ? (<ul className="space-y-2">{car[feature.key].map((item, i) => <li key={i} className="flex items-start"><span className="mr-2 mt-1">&#8226;</span><span>{item}</span></li>)}</ul>) : (<span>{car[feature.key]}</span>)}</td>))}
                        </tr>
                    ))}
                    <tr className="bg-white">
                         <td className="sticky left-0 p-4 font-medium text-gray-800 align-top bg-white border-r border-gray-200">Comprar</td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 transition-transform transform hover:-translate-y-2">
                    ...
                    <form onSubmit={handleTaxCalculation}>
                        <input type="number" placeholder="Valor del avalúo (ej: 50000000)" value={avalúo} onChange={(e) => setAvalúo(e.target.value)} className="w-full p-3 mb-4 border border-gray-300 rounded-lg bg-gray-50 text-lg" />
                        <button type="submit" className="w-full bg-green-600 text-white font-bold py-3 rounded-lg hover:bg-green-700 transition-all transform hover:scale-105">Calcular</button>
                    </form>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 transition-transform transform hover:-translate-y-2">
                    ...
                    <a href="https://www.runt.com.co/consulta-ciudadana/#/consultaVehiculo" target="_blank" rel="noopener noreferrer" className="w-full bg-red-500 text-white font-bold py-3 rounded-lg hover:bg-red-600 flex items-center justify-center transition-all transform hover:scale-105">Ir al RUNT <ExternalLink size={16} className="ml-2"/></a>
                </div>
            </div>
        </main>
    );
...
const BlogPage = () => {
    ...
    useEffect(() => { setTimeout(() => { setArticles(mockArticles); setIsLoading(false); }, 1000); }, []);
